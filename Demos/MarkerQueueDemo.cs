...
public partial class MarkerQueueDemo : Form
{
    private int nextDataIndex = 0;

    // holds two singele Arrays of doubles for ploting two signals
    private readonly double[][] ploterData = new double[2][];

    private readonly PlotData plotData;


    public MarkerQueueDemo(PlotData PlotData)
    {
        InitializeComponent();
        plotData = PlotData;

        // create empty data arrays with length of the MaxIterations you want 
        for (int i = 0; i < 2; i++)
        {
    ...
    {
        // reads two double values from ploter queue
        // and plots them with every TryDequeueList
        ...
        for (int n = 0; n < 2; n++)
        {
            ...
            if (success)
            {
                for (int i = 0; i < ploterData.Length; i++)
                {
                    ...
                    formsPlotgl1.Plot.AddMarker(values[0], values[1], MarkerShape.cross, 6, Color.DarkBlue);
                }
                nextDataIndex += 1;
            }
            else
